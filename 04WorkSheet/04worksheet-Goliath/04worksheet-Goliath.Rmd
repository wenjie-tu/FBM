...
```{r}
demo <- inla.output$marginals.fixed$"(Intercept)"
```
...
FNN::KL.divergence(edfun(m_var[, 1])$rfun(1000), edfun(sigma2_gibbs_samples)$rfun(1000))
```

 - Firstly, by overlaying the histogram of samples obtained from Gibbs sampling with the density plot of samples obtained from INLA, we see that both results converge to the same target distribution (with trivial disagreement).
...
Due to the limited number of samples from INLA, we first fitted an empirical CDF to mu's Gibbs samples and INLA, respectively. Then we sample 100 samples from those two ecdf respectively. Finally we apply the function of kl.divergence to compare two distributions.
