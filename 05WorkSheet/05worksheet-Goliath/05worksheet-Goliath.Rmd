...
**Geweke**: The results of Geweke's convergence diagnostic for 4 chaines are all in the range of -2 to 2. However, the Geweke-Brooks plot indicates that the 4th chain has more samples who hold a Z-score that is not in the range of -2 to 2, which proves that the 4th chain has not reached equilibrium.

**Heidelberger & Welch**: Heidelberger and Welch's convergence diagnostic shows a similar result as Geweke-Brooks plot. (The variance in the 4th chain fails the stationary test indicating non convergence therefore a longer MCMC run is needed) -> Null hypothesis that the chain is from a stationary distribution is accepted across all four. All parameters (except the beforementioned one) pass the halfwidth test which means that the length of the sample is long enough to estimate the mean with sufficient accuracy.
...
**Trace plot** The traceplots show agian that all the four chains of three parameters hold good mixing (-> Maybe that they explore the distribution well?).
...
We keep the same model as before but we change `n.iter` from 10'000 to 30'000 and `n.thin` from 1 to 3, to reduce the autocorrelation. The number of burn-in iterations (`n.burin`= 4000) is kept. 
...
```
The obtained absolute values for $\alpha$ and $\beta$ are all below 2, which indicates convergence to the target distribution. The plot shows that for the some z-score points of the different chains lie above or below 2.0, but only a fe which is acceptable. 
