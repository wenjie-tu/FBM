...
<!-- **Confused finding/declaration**: although `set.seed(44566)` is implemented in every chunk that will generate random number, the results still change every time (due to update()/the burn-in). That is the reason why the results and interpretation are NOT consistent with compiled version. -->
...
```{r rjags, warning=FALSE, message=FALSE, out.width="70%", eval=FALSE}
...
The following lines of code provide us with a basic setup 
```{r introduction, fig.align='center', out.width="75%"}
set.seed(44566)
...
rg <- range(m_var[,2])
truehist(sigma2.sim, prob=TRUE, col="yellow", xlab=expression(sigma^2),ylim=rg, xlim=c(5,40))
...
The empirical mean and standard deviation for each variable and corresponding quantiles are shown in the output.
Traceplots and density plots of the four chain model show similar results when compared to the single chain model. Moreover we can find good mixing for the three parameters (mean, variance and precision); INLA and JAGS have similar results.

```{r coda, fig.show='hold'}
...
**ESS**: The Effective Sample Size of the three parameters are shown in the table. The ESS must be large enough to get stable inferences for quantities of interest. In the case of 10000 iterations, the global ESS of around 4000 is large enough. 
...
**Geweke**: The results of Geweke's convergence diagnostic for 4 chaines are all in the range of -2 to 2. (However, the Geweke-Brooks plot indicates that the 4th chain has more samples who hold a Z-score that is not in the range of -2 to 2, which proves that the 4th chain has not reached equilibrium.)**Newest pdf version doesnt support observations in brackets**

**Heidelberger & Welch**: Heidelberger and Welch's convergence diagnostic shows a similar result as Geweke-Brooks plot. (The variance in the 4th chain fails the stationary test indicating non convergence therefore a longer MCMC run is needed). All parameters (except the beforementioned one) pass the halfwidth test which means that the length of the sample is long enough to estimate the mean with sufficient accuracy. **Newest pdf version doesnt support observations in brackets**

**Raftery & Lewis** Raftery and Lewis's diagnostic gives the minimum sample size based on zero autocorrelation. All the four chains need a minimum sample size of 3746.

**Trace plot** Again the traceplots show that each of the four chains explores the distribution pretty good.

## runjags interface with a link to a file

```{r additional1, fig.show='hold'}
wb_data <- list( N=30, 
                 y=c(3.048,2.980,2.029,7.249,-0.259,3.061,4.059,6.370,7.902,1.926,
                     9.094,10.489,-0.384,-3.096,2.315,5.830,-1.542,-1.544,5.714,
                     -5.182,3.828,-4.038,2.169,5.087,-0.201,4.880,3.302,3.859,
...
**Autocorrelation plots** gives an overview of autocorrelation in different thinning interval. For all three parameters, when the lag is larger than 1, the autocorrelation is close to 0.

**Cross-correlation plot** shows the correlation between parameters. The correlation between variance and mean is close to 0, while variance and precision have a strong negative correlation which is not surprising given that the precision is the inverse of the variance.
...
The function `R2jags::jags` computes deviance when `DIC` is `true`, where DIC is an estimate of expected predictive error and lower deviance is better. Besides, using `plot` function, the quantiles for each variable are visualized. 
...
mcmc_rank_hist(fit.bayesian)
```

```{r, fig.show='hold', out.width="25%", include=FALSE, eval=FALSE}
## Rank plot of alpha
...
```{r}
heidel.diag(fit.bayesian)
```

The output from `heidel.diag` shows the summary results for the four generated chains.
The diagnostic on one hand checks if the length of the sample is long enough and on the other hand checks if the means are estimated from a converged chain. 
...
```{r}
gelman.diag(fit.bayesian, autoburnin=TRUE)
```
...
- Raftery \& Lewis: The dependence factors for the four chains are larger than 5 and hence autocorrelation exists. 
...
In the following steps the MCMC simulation is adapted to the findings from sub task 5.2.
We keep the same model as before but we change `n.iter` from 10'000 to 30'000 and `n.thin` from 1 to 3, to reduce the autocorrelation. The number of burn-in iterations (`n.burnin`= 4000) is kept. 
...
update(model2.jags, n.iter = 6000)
...
Compared to the rank plots obtained with the original model, the rank plots for $\beta$ in the new model have a higher resamblance with a normal distribution. The rank plots of $\alpha$ are comparable. 
...
The p-values of the Heidelberg diagnostics have slightly changed. But the interpretation stays the same above. 
...
The values for the dependence factor $I$ have increased, so we obtain an even stronger autocorrelation than before. 

```{r}
## Geweke (Convergence to stationarity)
geweke.diag(fit2.jags.coda)
```
The obtained absolute values for $\alpha$ and $\beta$ are all below 2, which indicates convergence to the target distribution. The plot shows that some z-score points of the different chains lie above or below 2.0, but only a few which is acceptable. 
...
The Gelman and Rubin diagnostic output is equivalent to the one from the original model. 
...
The univariate potential scale reduction factors for both alpha and beta are close to 1. Compared to the original model the effective sample size has been increased. 
...
As expected, the effective sample size is with both methods higher in the model with thinning = 10 compared to thinning = 1.
