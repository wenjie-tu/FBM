...
## Compute standard deviation, posterior mean and precision
std_dev <- sqrt((1/lambda) + (1/kappa))
mu.post     <- (kappa*n*y.bar + lambda*mu) / (n*kappa + lambda)
lambda.post <- n*kappa + lambda
```

So the expectation for $y$ equals $\mu = 161$ and the standard deviation is $\sqrt{\lambda^{-1}+\kappa^{-1}} =$  `r format(std_dev, digits = 4)`.
...
* In the Gamma ($\text{shape}=a,\text{rate}=b$) distribution, the mean and median do not coincide. This can be explained with the Jensens Inequality. It says that for a **convex** function $\rho$ it holds $\rho(E(X))\leq E(\rho(X))$. Since the inverse- and squared-inverse functions are both convex functions, this can be applied. 
